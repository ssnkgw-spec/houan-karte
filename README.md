...
# カルテ執筆は Claude Code の対話セッションで /gather-sources → /create-karte（APIキー不要）
...
一次資料の収集（Phase A）から下書き（B）・人間ゲート（C）までを Claude Code の対話セッションで回す（サブスク課金・APIキー不要）。

**Phase A 収集** — `/gather-sources <法案名 or bill-id>`
- `.claude/skills/gather-sources/SKILL.md` の手順で、Claude が **WebSearch/WebFetch を `lib/primary-sources.ts` の許可ドメインに限定**して収集し、8セクションに割り付けた `work/素材-<id>.md`（git 管理外）を生成する。報道・政党サイトは素材にしない。

**Phase B 生成〜確定** — `/create-karte <bill-id>`
- セッション内で下書き `content/bills/<id>.draft.ts` を生成 → サブエージェントの敵対的チェック（`work/review-report-<id>.md`）→ バリデーションループ（typecheck・中立リント・構造・HTMLプレビュー、最大5回自動修正）。

**Phase C 人間ゲート（§6-3）と確定**
1. `work/review-report-<id>.md` 末尾の4項目チェックと `work/preview-<id>.html` を人が確認
2. チャットで「承認」→ `content/bills/<id>.ts` 確定・`index.ts` 登録・commit・push まで自動実行
...
- 執筆（/create-karte）・収集（/gather-sources）は **Claude Code の対話セッション**、審査（triage）は **`claude -p`＝Claude Code CLI** で動く（いずれもサブスク課金）。**Anthropic API キーは不要**（コードからキー依存を排除済み）。Actions も議案DB取得と状況変化検知だけで LLM を使わない
